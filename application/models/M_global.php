...
class M_global extends CI_Model
{
	...
	{
		$query = $this->db->query(" SELECT t.id, t.kode_transaksi, t.tanggal, gd.id_game_detail, gd.nama_game_detail, t.no_telp, t.harga_item, t.status FROM `transaction` t JOIN game_detail gd ON t.id_game_detail = gd.id_game_detail WHERE kode_transaksi = '$param' OR no_telp = '$param' ORDER BY id DESC");
		return $query->result();
	}
	public function getHistoryForAdmin($param)
	{
		$query = $this->db->query(" SELECT t.id, t.kode_transaksi, t.tanggal, gd.id_game_detail, gd.nama_game_detail, t.no_telp, t.harga_item, t.status FROM `transaction` t JOIN game_detail gd ON t.id_game_detail = gd.id_game_detail WHERE kode_transaksi = '$param' ORDER BY id DESC");
		return $query->result();
	}

	public function updateStatusTrans($kodeTrans, $data)
	{
		$this->db->where('kode_transaksi', $kodeTrans);
